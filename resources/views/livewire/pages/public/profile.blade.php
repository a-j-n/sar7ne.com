<div class="space-y-12 animate-fade-in-up" wire:ignore.self>
    @php
    ...
    @endphp
        ...
        <section class="relative overflow-hidden rounded-3xl border border-primary bg-primary shadow-2xl">
            ...
            <!-- Content -->
            <div class="relative flex flex-col gap-8 p-8 text-center md:flex-row md:items-center md:text-left">
                <!-- Profile Image with Enhanced Styling -->
                <div class="relative mx-auto md:mx-0">
                    <div class="absolute -inset-2 rounded-3xl bg-gradient-orange-pink opacity-75 blur-lg"></div>
                    <img src="{{ $user->avatarUrl() }}" alt="{{ $user->username }} avatar" 
                         class="relative h-36 w-36 md:h-40 md:w-40 rounded-3xl object-cover cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-2xl border-4 border-white/20" 
                         data-profile-image />
                    <!-- Online Status Indicator -->
                    <div class="absolute -bottom-2 -right-2 h-8 w-8 rounded-full bg-brand-mint border-4 border-primary shadow-xl">
                ...
                <div class="flex-1 space-y-4">
                    ...
                    <div class="space-y-3">
                        <h1 class="text-5xl md:text-6xl font-black bg-gradient-orange-pink bg-clip-text text-transparent leading-tight tracking-tight">
                            {{ "@".$user->username }}
                        </h1>
                        @if ($user->display_name)
                            <p class="text-xl font-semibold text-secondary">{{ $user->display_name }}</p>
                        @endif
                    </div>
                    
                    <!-- Bio -->
                    <div class="relative">
                        <p class="text-lg text-secondary leading-relaxed max-w-2xl font-medium">
                            {{ $user->bio ?? __('messages.drop_anonymous_message') }}
                        </p>
                        <!-- Decorative underline -->
                        <div class="absolute -bottom-2 left-0 w-16 h-1 bg-gradient-orange-pink rounded-full opacity-60"></div>
                    </div>
                    
                    <!-- Stats Row -->
                    <div class="flex flex-wrap items-center justify-center md:justify-start gap-6 text-sm">
                        <div class="flex items-center gap-2 px-3 py-1 rounded-full bg-secondary/50">
                    ...
                    <div class="flex justify-center md:justify-start mt-6">
                        <button onclick="shareProfile()" 
                                class="group inline-flex items-center gap-3 px-6 py-3 text-sm font-semibold text-white bg-gradient-orange-pink rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 glow-brand-orange">
            ...
            <div class="relative p-8">
                ...
                <div class="text-center mb-10">
                    <div class="relative inline-block mb-6">
                        ...
                        <div class="relative inline-flex items-center gap-4 px-8 py-4 rounded-2xl bg-gradient-orange-pink/10 border-2 border-brand-orange/20 shadow-lg">
                            <div class="w-3 h-3 rounded-full bg-brand-orange animate-pulse shadow-lg"></div>
                            <h2 class="text-2xl font-black text-primary tracking-tight">{{ __('messages.send_anonymous_message') }}</h2>
                        </div>
                    </div>
                    <p class="text-base text-tertiary max-w-lg mx-auto leading-relaxed">{{ __('messages.identity_notice') }}</p>
                </div>
                ...
                <form class="space-y-6" method="POST" action="{{ $messageRoute }}" enctype="multipart/form-data">
                    ...
                    <div class="relative">
                        <textarea id="message" name="message_text" rows="6" maxlength="500" 
                                  ...
                                  class="w-full rounded-2xl border-2 border-secondary bg-primary px-6 py-4 text-base text-primary placeholder:text-muted focus:border-brand-orange focus:ring-4 focus:ring-brand-orange/10 focus:outline-none transition-all duration-300 resize-none"
                    ...
                    <div class="space-y-3">
                        <label for="messageImage" 
                               class="group relative flex items-center justify-center gap-3 px-6 py-4 text-sm font-medium text-secondary bg-secondary/30 border-2 border-dashed border-secondary rounded-2xl hover:bg-secondary/50 hover:border-brand-orange transition-all duration-300 cursor-pointer">
                        ...
                        <div id="imagePreview" class="hidden rounded-2xl overflow-hidden border-2 border-secondary">
                            ...
                            <div class="p-3 bg-secondary/50 flex justify-between items-center">
                    ...
                    <button type="submit" 
                            class="group relative w-full overflow-hidden rounded-2xl bg-gradient-orange-pink px-8 py-4 text-base font-bold text-white shadow-2xl transition-all duration-300 hover:scale-[1.02] hover:shadow-3xl glow-brand-orange">
                ...
                <div class="mt-6 text-center">
                    <p class="text-xs text-muted bg-secondary/30 px-4 py-2 rounded-full inline-block">
            ...
            <div class="relative p-8">
                ...
                <div class="flex items-center gap-4 mb-8">
                    <div class="flex items-center gap-3">
                        ...
                        <h2 class="text-2xl font-bold text-primary">{{ __('messages.public_messages') }}</h2>
                    </div>
                    <div class="flex-1 h-px bg-gradient-to-r from-brand-mint/50 to-transparent"></div>
                    <div class="px-3 py-1 rounded-full bg-brand-mint/10 border border-brand-mint/20">
                        <span class="text-sm font-medium text-brand-mint">{{ $publicMessages->count() }} {{ $publicMessages->count() === 1 ? 'message' : 'messages' }}</span>
                    </div>
                </div>
                
                <!-- Messages Grid -->
                <div class="grid gap-6" data-section="public-messages">
                    @foreach ($publicMessages as $index => $msg)
                        <article class="group relative overflow-hidden rounded-2xl border border-secondary bg-secondary/30 p-6 transition-all duration-300 hover:shadow-xl hover:scale-[1.02] hover:border-brand-mint/50"
                            ...
                            <div class="space-y-4">
                                ...
                                <!-- Message Footer -->
                                <div class="flex items-center justify-between pt-2 border-t border-secondary/50">
