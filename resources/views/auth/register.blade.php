    ...
    <div class="mx-auto max-w-lg space-y-6">
        ...
        <form method="POST" action="{{ route('register.store') }}" class="space-y-3 rounded-2xl border border-white/10 bg-white/5 p-6">
            ...
            <div class="text-left">
                ...
                <input id="email" name="email" type="email" value="{{ old('email') }}" required autocomplete="email" class="w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-sm text-white placeholder-slate-400 outline-none focus:border-white/30" placeholder="{{ __('messages.email_placeholder') }}">
                @error('email')
