...
@section('content')
    <div class="mx-auto max-w-lg space-y-6">
        <div class="rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 p-8 text-center shadow-xl">
        ...
        <div class="space-y-4">
            ...
            <a href="{{ route('oauth.redirect', ['provider' => 'twitter']) }}" class="flex items-center justify-center gap-3 rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/10 px-6 py-4 text-sm font-medium text-slate-900 dark:text-white transition hover:bg-slate-50 dark:hover:bg-white/20">
            ...
            <form method="POST" action="{{ route('login.attempt') }}" class="space-y-3 rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 p-6">
                @csrf
                <div class="text-left">
                    <label for="email" class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300">{{ __('messages.email_label') }}</label>
                    <input id="email" name="email" type="email" value="{{ old('email') }}" required autocomplete="email" autofocus class="w-full rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-black/20 px-3 py-2 text-sm text-slate-900 dark:text-white placeholder-slate-500 dark:placeholder-slate-400 outline-none focus:border-slate-300 dark:focus:border-white/30" placeholder="{{ __('messages.email_placeholder', ['example' => 'you@example.com']) }}">
                    @error('email')
                ...
                <div class="text-left">
                    ...
                    <input id="password" name="password" type="password" required autocomplete="current-password" class="w-full rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-black/20 px-3 py-2 text-sm text-slate-900 dark:text-white placeholder-slate-500 dark:placeholder-slate-400 outline-none focus:border-slate-300 dark:focus:border-white/30" placeholder="••••••••">
                    @error('password')
                        <p class="mt-1 text-xs text-red-300">{{ $message }}</p>
                    @enderror
                </div>
                <div class="flex items-center justify-between">
                    <label class="flex items-center gap-2 text-xs text-slate-700 dark:text-slate-300">
                        <input type="checkbox" name="remember" value="1" {{ old('remember') ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 dark:border-white/20 bg-white dark:bg-black/20">
                        {{ __('messages.remember_me') }}
                    </label>
                    <button type="submit" class="rounded-xl bg-slate-900 dark:bg-white px-4 py-2 text-xs font-medium text-white dark:text-black transition hover:bg-black dark:hover:bg-slate-200">{{ __('messages.sign_in_with_email') }}</button>
                </div>
            </form>
